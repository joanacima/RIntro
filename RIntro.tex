...
\NormalTok{  tbl }\OtherTok{\textless{}{-}} \FunctionTok{table}\NormalTok{(x, }\AttributeTok{useNA =} \StringTok{"no"}\NormalTok{)  }\CommentTok{\# ignora NAs}
\NormalTok{  max\_freq }\OtherTok{\textless{}{-}} \FunctionTok{max}\NormalTok{(tbl)}
\NormalTok{  mode\_val }\OtherTok{\textless{}{-}} \FunctionTok{as.numeric}\NormalTok{(}\FunctionTok{names}\NormalTok{(tbl)[tbl }\SpecialCharTok{==}\NormalTok{ max\_freq])}
  \ControlFlowTok{if}\NormalTok{(}\FunctionTok{length}\NormalTok{(mode\_val) }\SpecialCharTok{\textgreater{}} \DecValTok{1}\NormalTok{) \{}
    \FunctionTok{warning}\NormalTok{(}\StringTok{"Existem múltiplas modas. Selecionando a menor."}\NormalTok{)}
\NormalTok{    mode\_val }\OtherTok{\textless{}{-}} \FunctionTok{min}\NormalTok{(mode\_val)}
\NormalTok{  \}}
  \FunctionTok{return}\NormalTok{(mode\_val)}
\NormalTok{\}}

    \DocumentationTok{\#\#\# compute the mode}
\NormalTok{mode\_union }\OtherTok{\textless{}{-}} \FunctionTok{calc\_mode}\NormalTok{(nlswork}\SpecialCharTok{$}\NormalTok{union)}

    \DocumentationTok{\#\#\# Replace NA by mode in column union}
\NormalTok{  nlswork\_filled }\OtherTok{\textless{}{-}}\NormalTok{ nlswork }\SpecialCharTok{\%\textgreater{}\%}
  \FunctionTok{mutate}\NormalTok{(}\AttributeTok{union =} \FunctionTok{if\_else}\NormalTok{(}\FunctionTok{is.na}\NormalTok{(union), mode\_union, union))}
\end{Highlighting}
\end{Shaded}
